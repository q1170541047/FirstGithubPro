...
import com.yiche.baselibrary.util.ARouter.ARouterUtil;
...
/**
 * Created by changhe_li on 2019/11/29 10:33.
 * 描述：
 */
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (logHeaders) {
            ...
            if (!logBody || !hasRequestBody) {
                ...
                logger.log("");
                if (requestBody.contentLength() < 20 * 1024) {
        ...
        panduan_status(response, request.url().toString());
...
//        return response;
        if (!myresponseBody.isEmpty()) {
            ...
            return response.newBuilder().body(ResponseBody.create(mediaType, myresponseBody)).build();
    ...
    private void panduan_status(Response response, String requestUrl) {
        try {

            ResponseBody responseBody2 = response.body();
            BufferedSource source2 = responseBody2.source();
            source2.request(Long.MAX_VALUE);
            String respString = source2.buffer().clone().readString(Charset.defaultCharset());
            if (respString.contains("error_no")) {
                Gson gson = new Gson();
                BaseBean baseBean = gson.fromJson(respString, BaseBean.class);
                if (baseBean.getError_no().equals("402")) {
                    onTokenOverdue(baseBean.getError_msg(), requestUrl);
                }
//                if (baseBean.getError_no().equals("200")) {
//                    myresponseBody = "{\"error_no\":\"" + 201 + "\",\"error_msg\":\"" + "111" + "\",\"result\":{\"ok\":\"ok\"}}";
//                }
            }
        } catch (Exception e) {
    ...
    protected void onTokenOverdue(String msg, String requestUrl) {
//        MyToastUtil.showToast(msg);
        try {
...
//                }
//            });
            String name = curActivity.getLocalClassName();
            if (!name.contains("LoginActivity")) {
                ARouterUtil.jumpToActivity("//登录");
            }
        } catch (Exception e) {
